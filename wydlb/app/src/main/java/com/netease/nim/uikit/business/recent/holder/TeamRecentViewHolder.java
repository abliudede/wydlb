...
public class TeamRecentViewHolder extends CommonRecentViewHolder {
    ...
    protected String getContent(RecentContact recent) {
        ...
        if (!TextUtils.isEmpty(fromId)
            ...
            //有人艾特你和有人回复你文案
            if (TeamMemberAitHelper.hasReplyExtension(recent)) {
                if (recent.getUnreadCount() == 0) {
                    TeamMemberAitHelper.clearRecentContactReplyed(recent);
                } else {
                    content = TeamMemberAitHelper.getReplyAlertString(content);
                }
            }else  if (TeamMemberAitHelper.hasAitExtension(recent)) {
                if (recent.getUnreadCount() == 0) {
