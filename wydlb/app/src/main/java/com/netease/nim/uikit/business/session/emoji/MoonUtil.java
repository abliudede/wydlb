...
public class MoonUtil {
    ...
    public static void identifyRecentVHFaceExpressionAndTags(Context context, View textView,
                                                             String value, int align, float scale,int in) {
        ...
        if(in >0) {
            URLUtils.replaceWeborBook(value, mSpannableString, context, in);
        }
        viewSetText(textView, mSpannableString);
        if(in > 0) {
            // 在单击链接时凡是有要执行的动作，都必须设置MovementMethod对象
            ((TextView) textView).setMovementMethod(CustomLinkMovementMethod.getInstance());
        }
    }
