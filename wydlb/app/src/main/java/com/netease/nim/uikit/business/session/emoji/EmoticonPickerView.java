...
public class EmoticonPickerView extends LinearLayout implements IEmoticonCategoryChanged {
    ...
    private CheckedImageButton addEmoticonTabBtn(int index, OnClickListener listener) {
        ...
        emotBtn.setPaddingValue(ScreenUtil.dip2px(5));
    ...
    private void setSelectedVisible(final int index) {
        ...
        uiHandler.postDelayed(new MyRunnable(this,index), 100);
    }

    private static class MyRunnable implements Runnable{
        int mIndex;
        WeakReference<EmoticonPickerView>reference;

        public MyRunnable(EmoticonPickerView emoticonPickerView,int index) {
            reference=new WeakReference<>(emoticonPickerView);
            mIndex=index;
        }

        @Override
        public void run() {
            if (reference.get().scrollView.getChildAt(0).getWidth() == 0) {
                reference.get().uiHandler.postDelayed(this, 100);
            }
            int x = -1;
            View child = reference.get().getChildAt(mIndex);
            if (child != null) {
                if (child.getRight() >reference.get().scrollView.getWidth()) {
                    x = child.getRight() - reference.get().scrollView.getWidth();
                }
            }
            if (x != -1) {
                reference.get().scrollView.smoothScrollTo(x, 0);
            }
        }
    }
