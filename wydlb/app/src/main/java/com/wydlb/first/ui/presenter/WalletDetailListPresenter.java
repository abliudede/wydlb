...
import com.wydlb.first.ui.contract.WalletDetailListContract;
...
public class WalletDetailListPresenter extends RxPresenter<WalletDetailListContract.View> implements WalletDetailListContract.Presenter<WalletDetailListContract.View> {
    ...
    @Override
    public void getBalancePage(ArrayMap<String,Object>maps) {
        ...
        Disposable disp =mReaderApi.getBalancePage(maps).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                mView.getBalancePageSuccess(data);
                            }else{
