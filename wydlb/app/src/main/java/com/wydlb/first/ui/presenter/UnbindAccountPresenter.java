...
import com.wydlb.first.ui.contract.UnbindAccountContract;
...
public class UnbindAccountPresenter extends RxPresenter<UnbindAccountContract.View> implements UnbindAccountContract.Presenter<UnbindAccountContract.View> {
    ...
    @Override
    public void unBindAccount(ArrayMap<String,Object> params) {
        ...
        Disposable disp =mReaderApi.unBindAccount(params).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.unBindAccountSuccess(data);
                                }else{
