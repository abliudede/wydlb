...
public class UpdateMessageStatusPresenter extends RxPresenter<UpdateMessageStatusContract.View> implements UpdateMessageStatusContract.Presenter<UpdateMessageStatusContract.View> {
    ...
    @Override
    public void updateMessageStatus(ArrayMap<String, String> maps,boolean isPrivate) {
        ...
        Disposable disp =mReaderApi.updateMessageStatus(maps,isPrivate).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.updateMessageStatusSuccess(data);
                                }else{
