...
import static com.wydlb.first.utils.RxConstTool.BYTE;
import static com.wydlb.first.utils.RxConstTool.GB;
import static com.wydlb.first.utils.RxConstTool.KB;
import static com.wydlb.first.utils.RxConstTool.MB;
...
public class RxDataTool {
    ...
    private static final DecimalFormat amountFormat = new DecimalFormat("###,###,###,##0.00");//READ
    private static final DecimalFormat amountCNYFormat = new DecimalFormat("###,###,###,##0.00");
    ...
    public static String getLikeCount(int count){
        if(count <= 0){
            return "";
        }else if(count <= 999){
    ...
    public static String hideMobilePhone4(String mobile_phone) {
        if (mobile_phone.length() != 11) {
            return mobile_phone.substring(0, mobile_phone.length()-2) + "****" ;
        }else{
...
//        }
        return calculateProfit(stringToDouble(str));
    }
     ...
     * @return
     */
    public static String format1Decimals(double doubleValue) {
        // 保留4位小数
        java.text.DecimalFormat df = new java.text.DecimalFormat("#.00000000");
        String result = df.format(doubleValue);
        // 截取第一位
        ...
        // 字符串截断
        return result.substring(0, inde + 2);
    }
    ...
    public static String getCNYAmountValue(String value) {
        if (TextUtils.isEmpty(value)) {
            return "0";
        }
        return amountCNYFormat.format(Double.parseDouble(value));
    }
     ...
     * @return
     */
    public static String getRoundDown(BigDecimal value, int digit) {
    ...
    public static String getPercentValue(BigDecimal value, int digit) {
        ...
        return getRoundDown(result, digit);
    }
...
//        Double bigDecimal = new Double(name*100);     存在精度问题 eg：145296.8
    ...
    public static String getOrderNo(){//A+时间yyyyMMddHHmmss+时间戳前7位 共计24位
        return "AN"+RxTimeTool.getOrderDateStr()+String.valueOf(System.currentTimeMillis()).substring(7);
    }
     ...
     * @return
     */
    public static String getRechargeOrderNo(){//A+时间yyyyMMddHHmmss+时间戳前7位 共计24位
        return "AR"+RxTimeTool.getOrderDateStr()+String.valueOf(System.currentTimeMillis()).substring(7);
    }
     ...
     * @return
     */
    public static String getExchargeOrderNo(){//A+时间yyyyMMddHHmmss+时间戳前7位 共计24位
        return "AE"+RxTimeTool.getOrderDateStr()+String.valueOf(System.currentTimeMillis()).substring(7);
    }
    /**
     * 获取兑换订单
     * @return
     */
    public static String getWithdrawOrderNo(){//A+时间yyyyMMddHHmmss+时间戳前7位 共计24位
        return "AW"+RxTimeTool.getOrderDateStr()+String.valueOf(System.currentTimeMillis()).substring(7);
    }
    ...
    public static String getFormatNumber(int number){
        if(number <= 0){
            ...
            double kcount = (double)number/1000;
            return format1Decimals(kcount)+"千";
        }else {
            double mcount = (double)number/10000;
            return format1Decimals(mcount)+"万";
        }
    }
