...
import com.wydlb.first.ui.contract.WalletBalanceContract;
...
public class WalletBalancePresenter extends RxPresenter<WalletBalanceContract.View> implements WalletBalanceContract.Presenter<WalletBalanceContract.View> {
    ...
    @Override
    public void getWalletBalance(ArrayMap<String,Object> maps) {
        ...
        Disposable disp =mReaderApi.getAccountBalance(maps).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.getWalletBalanceSuccess(data);
                                }else{
