...
import com.wydlb.first.ui.contract.AuthInfoContract;
...
public class AuthInfoPresenter extends RxPresenter<AuthInfoContract.View> implements AuthInfoContract.Presenter<AuthInfoContract.View> {
    ...
    public void getAuthInfo() {
        ...
        Disposable disp =mReaderApi.getAuthInfo().compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.getAuthInfoSuccess(data);
                                }else{
...
//                            if (!e.toString().contains("NullPointerException")){
                                try{
                                    ...
                                    RxLogTool.e("AuthInfoPresenter:"+ex.toString());
                                }
//                            }
                        }
                );
