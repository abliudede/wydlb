...
import com.wydlb.first.ui.contract.PostPublishContract;
...
public class PostPublishPresenter extends RxPresenter<PostPublishContract.View> implements PostPublishContract.Presenter<PostPublishContract.View> {

    private ReaderApi mReaderApi;

    @Inject
    public PostPublishPresenter(ReaderApi readerApi) {
        this.mReaderApi = readerApi;
    }

    @Override
    public void postPublish(String bodyJson) {
        if (!RxNetTool.isAvailable()){
            mView.showError(Constant.NetWorkStatus.NETWORK_UNAVAILABLE);
            return;
        }
        Disposable disp =mReaderApi.postPublish(bodyJson).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.postPublish(data);
                                }else{
