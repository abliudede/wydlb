...
import com.wydlb.first.ui.contract.UserLockListContract;
...
public class UserLockListPresenter extends RxPresenter<UserLockListContract.View> implements UserLockListContract.Presenter<UserLockListContract.View> {

    private ReaderApi mReaderApi;

    @Inject
    public UserLockListPresenter(ReaderApi readerApi) {
        this.mReaderApi = readerApi;
    }

    @Override
    public void userLockList(ArrayMap map) {
        ...
        Disposable disp =mReaderApi.getUserLockList(map).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.userLockListSuccess(data);
                                }else{
