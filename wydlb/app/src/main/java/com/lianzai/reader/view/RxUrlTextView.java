...
public class RxUrlTextView extends AppCompatTextView {
    public RxUrlTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public void setUrlText(String text,final Context context){
        ...
        for(int i=0;i<urlCount;i++){
            ...
            spannableString.setSpan(new ClickableSpan() {
                ...
                public void onClick(View widget) {
                    ...
                    intent.putExtra(ActivityWebView.PARAM_URL, href);
            ...
            RxLogTool.e("起始位置"+(end-lastStart));
            remainText = remainText.substring(end-lastStart,remainText.length());//剩下的部分
            lastStart = end;
            RxLogTool.e("front是"+front);
            RxLogTool.e("spann是"+spannableString);
            RxLogTool.e("remain是"+remainText);
    ...
    public static Object[][]  getUrlFromJDP(String source){
        ...
        while(matcher.find()){
            ...
            RxLogTool.e("蓝色文字是"+host);//括号里面的
            hosts.add(i,host);
            starts.add(i,matcher.start());
            ends.add(i,matcher.end());
            RxLogTool.e("字符串起始下标是"+matcher.start()+"结尾下标是"+matcher.end());//匹配出的字符串在源字符串的位置
            i++;
        }
        if(hosts.size()==0){
            RxLogTool.e("没有发现url");
