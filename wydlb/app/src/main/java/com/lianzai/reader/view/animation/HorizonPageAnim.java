...
public abstract class HorizonPageAnim extends PageAnimation{
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (Constant.isListenBook){//听书模式
            RxLogTool.e("onTouchEvent....isListenBookStyle true");
            return false;
        }

        if (Constant.isAutoPage){//自动翻页模式
            RxLogTool.e("onTouchEvent....isListenBookStyle true");
            return false;
        }
        ...
        switch (event.getAction()){
                ...
                if (isMove){
                    ...
                    if (mMoveX == 0 && mMoveY ==0) {
                        ...
                        if (x - mStartX > 0){
                            ...
                            //如果不存在表示没有下一页了
                            if (!hasNext) {
                                RxEventBusTool.sendEvents(Constant.EventTag.SKIP_TO_CIRCLE);
                                noNext = true;
                                return true;
                            }
                        }
            ...
            case MotionEvent.ACTION_UP:
                if (!isMove){
                    if (ReadSettingManager.getInstance().isSingleHand()){//单手模式
                        isNext=true;//点击中心区域外的所有地方都是翻下一页
                    }else{
                        if (x < mScreenWidth / 2){
                            isNext = false;
                        }else{
