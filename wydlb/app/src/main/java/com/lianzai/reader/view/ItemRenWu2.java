...
public class ItemRenWu2 extends RelativeLayout{
	...
	public ItemRenWu2(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.context = context;
		initView();
	}

	public ItemRenWu2(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
		initView();
	}

	public ItemRenWu2(Context context) {
		super(context);
		this.context = context;
		initView();
	}
	...
	public void bindData(TaskCenterBean.DataBean.DailyTaskListBean bean) {
		...
		if(TextUtils.isEmpty(bean.getTaskIntro())){
			...
			if(bean.getZhankai() == 0){
				//收起的状态
				zhankai_iv.setImageResource(R.mipmap.renwu_down);
				zhankai_iv.setVisibility(View.VISIBLE);
			}else{
				//展开的状态
				zhankai_iv.setImageResource(R.mipmap.renwu_up);
				zhankai_iv.setVisibility(View.VISIBLE);
			}
			setOnClickListener(
					v -> {
