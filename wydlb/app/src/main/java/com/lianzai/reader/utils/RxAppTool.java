...
public class RxAppTool {
    ...
    public static void adSkip(BannerBean.DataBean bannerBean,Context mContext){
        if(null == bannerBean) return;
        if (TextUtils.isEmpty(bannerBean.getBannerUrl()))return;
        //假如是需要登录，先判断登录
        ...
        if(bannerBean.getType() == 8){
            ...
            if("money_reward_one".equals(bannerBean.getBannerUrl())){
                //跳转到一键赏
                if (RxLoginTool.isLogin()){
                    RxActivityTool.skipActivity(mContext,ActivityOneKeyReward.class);
                }else {
                    ActivityLoginNew.startActivity(mContext);
                }
            }else if("money_reward_pool".equals(bannerBean.getBannerUrl())){
                //跳转到赏金猎人
                if (RxLoginTool.isLogin()){
                    RxActivityTool.skipActivity(mContext,ActivityBountyHunter.class);
                }else {
                    ActivityLoginNew.startActivity(mContext);
                }
            }
        ...
        //判断是否是特殊游戏
        if(bannerBean.getIsSpecialGame() > 1){
            //特殊游戏
            ActivityWebView.startActivityForGame(mContext,bannerBean.getBannerUrl(),bannerBean.getIsSpecialGame());
        } else if(bannerBean.getIsSpecialGame() == 1) {
            //普通网页不显示更多
            ActivityWebView.startActivity(mContext,bannerBean.getBannerUrl(),1);
        } else if(bannerBean.getIsSpecialGame() == 0) {
            //普通网页
            ActivityWebView.startActivity(mContext,bannerBean.getBannerUrl(),0);
        }else if(bannerBean.getIsSpecialGame() == -1) {
            //跳到外部链接
            try{
                Intent intent = new Intent();
                intent.setData(Uri.parse(bannerBean.getBannerUrl()));
    ...
    public static boolean isCircle(int id){
        if(id == 1 || id == 24 || (id >= 50 && id <= 60)){
            return true;
        }else {
            return false;
        }
    ...
    public static void installApk(Context context, File file) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            Uri apkUri = FileProvider.getUriForFile(context, "com.lianzai.reader.TTFileProvider", file);
    ...
    public static boolean isAppForeground(Context context, String packageName) {
        return !TextUtils.isEmpty(packageName) && packageName.equals(RxProcessTool.getForegroundProcessName(context));
    }
    //----------------------------------------------------------------------------------------------------------------
    ...
    public static boolean isInstallApp(Context context, String packageName) {
        return !TextUtils.isEmpty(packageName) && RxIntentTool.getLaunchAppIntent(context, packageName) != null;
    }
