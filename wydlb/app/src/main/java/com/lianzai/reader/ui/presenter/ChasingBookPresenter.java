...
import com.lianzai.reader.ui.contract.ChasingBookContract;
...
public class ChasingBookPresenter extends RxPresenter<ChasingBookContract.View> implements ChasingBookContract.Presenter<ChasingBookContract.View> {
    ...
    @Override
    public void bookChasingList(ArrayMap<String,String> maps) {
        if (!RxNetTool.isAvailable()){
            mView.showError(Constant.NetWorkStatus.NETWORK_UNAVAILABLE);
            return;
        }

        Disposable disp =mReaderApi.bookChasingList(maps).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getStatus_code()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.bookChasingList(data);
                                }else{
