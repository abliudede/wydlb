...
import com.lianzai.reader.interfaces.IWsManager;
...
public class WsManager implements IWsManager {
    ...
    private WebSocketListener mWebSocketListener = new WebSocketListener() {
        ...
        public void onOpen(WebSocket webSocket, final Response response) {
            ...
            if (Looper.myLooper() != Looper.getMainLooper()) {
                sendMessage("[{\"process\": \"latestPrice\"}]");
            } else {
                RxLogTool.e("websocket", "服务器连接成功");
            }
        }
        ...
        public void onMessage(WebSocket webSocket, final ByteString bytes) {
            if (Looper.myLooper() != Looper.getMainLooper()) {
                wsMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        RxLogTool.e("websocket", "WsManager-----onMessage");
                    }
                });
            } else {
                RxLogTool.e("websocket", "WsManager-----onMessage");
        ...
        public void onClosing(WebSocket webSocket, final int code, final String reason) {
            if (Looper.myLooper() != Looper.getMainLooper()) {
                wsMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        RxLogTool.e("websocket", "服务器连接关闭中");
                    }
                });
            } else {
                RxLogTool.e("websocket", "服务器连接关闭中");
        ...
        public void onClosed(WebSocket webSocket, final int code, final String reason) {
            ...
            if (Looper.myLooper() != Looper.getMainLooper()) {
                wsMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        RxLogTool.e("websocket", "服务器连接已关闭");
                    }
                });
            } else {
                RxLogTool.e("websocket", "服务器连接已关闭");
        ...
        public void onFailure(WebSocket webSocket, final Throwable t, final Response response) {
            try {
                tryReconnect();
                RxLogTool.e("liusehngjei", "[走的链接失败这里！！！！！！！！！！！！！！！！]");
                if (Looper.myLooper() != Looper.getMainLooper()) {
                    wsMainHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            RxLogTool.e("websocket", "服务器连接失败");
                        }
                    });
                } else {
                    RxLogTool.e("websocket", "服务器连接失败");
    ...
    private void tryReconnect() {
        ...
        RxLogTool.e("liusehngjei", "reconnectCount2222222[" + reconnectCount + "]");
        if (!NetworkUtil.isNetAvailable(mContext)) {
            setCurrentStatus(WsStatus.DISCONNECTED);
            RxLogTool.e("liusehngjei", "[请您检查网络，未连接]");
//            return;
        }
        setCurrentStatus(WsStatus.RECONNECT);
        RxLogTool.e("liusehngjei", "reconnectCount11111111[" + reconnectCount + "]");
...
//        wsMainHandler.postDelayed(reconnectRunnable, delay > RECONNECT_MAX_TIME ? RECONNECT_MAX_TIME : delay);
        ...
        RxLogTool.e("liusehngjei", "reconnectCount[" + reconnectCount + "]");
        reconnectCount++;
