 ...
 * 圈子-文章列表
 */
public class CircleArticleFragment extends BaseFragment implements NativeExpressAD.NativeExpressADListener{
...
////            overlapLogoView.dynamicUpdate(listTemp,"等"+ size +"条相关链接");
//
//            //判断是否是内站书，内站书隐藏相关链接的文案
////            if(getActivity() instanceof ActivityCircleDetail){
////                //圈子详情页
////                CircleInfoBean circleInfo = ((ActivityCircleDetail) getActivity()).getCircleInfoBean();
////                if(null != circleInfo && circleInfo.getData().getPlatformType() == Constant.UserIdentity.BOOK_PUBLIC_NUMBER) {
////                    overlapLogoView.setVisibility(View.GONE);
////                }
////            }
//
//        }catch (Exception e){
...
//    }
    ...
    public void circleDynamicRequest(boolean isRefresh){
        ...
        OKHttpUtil.okHttpPostJson(Constant.API_BASE_URL + "/circles/articleTabList", jsonObject.toString(), new CallBackUtil.CallBackString() {
            @Override
...
//                            initUpdateView(circleDynamicBean.getData().getExtObj());
                            ...
                            if(getActivity() instanceof ActivityCircleDetail){
                                //圈子详情页
                                ((ActivityCircleDetail)getActivity()).placedTopListRequest("3");
                            }
...
//                    e.printStackTrace();
    ...
    public void placedTopListRequest(String response){
        if(null == topHeaderView){
            topHeaderView = LayoutInflater.from(getActivity()).inflate(R.layout.head_circle_top,null);
            recyclerView = topHeaderView.findViewById(R.id.recyclerView);
            //将头部置顶模块添加进去
            if (null!=circleDynamicAdapter){
                circleDynamicAdapter.addHeaderView(topHeaderView,1);
            }
        }
                try{
        ...
        public void imageDynamicClick(int index,int pos) {
            ...
            try{
                ...
                if( listBean.getPostType() == 3){
                    ...
                    try{
                        CircleBookBean circleBookBean = GsonUtil.getBean(listBean.getPicturesShow(),CircleBookBean.class);
                        UrlReadActivity.startActivityInsideRead(getActivity(),circleBookBean.getBookId(),circleBookBean.getSource(),false,"",circleBookBean.getChapterName(),0,false);
                    }catch (Exception e){
                    }
