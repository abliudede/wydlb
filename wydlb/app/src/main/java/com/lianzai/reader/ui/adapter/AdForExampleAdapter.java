 ...
 * 百度腾讯广告加上首页动态
 */
public class AdForExampleAdapter extends BaseQuickAdapter<Object, BaseViewHolder> {
    ...
    public AdForExampleAdapter(@Nullable List<Object> data, Context mContext) {
        super(R.layout.item_ad_example_view, data);
        contentWidth = RxDeviceTool.getScreenWidth(mContext) - RxImageTool.dip2px(20);
    }
...
//        notifyItemRangeChanged(0, mData.size() - 1);
    ...
    protected void convert(final BaseViewHolder holder, Object obj) {
        ...
        if (TYPE_AD1 == type) {
            ...
            //圈子信息
            RelativeLayout dynamic_book_view = holder.getView(R.id.dynamic_book_view);
            ...
            OverlapLogoView olv_users_logo = holder.getView(R.id.olv_users_logo);
...
//            TextView tv_book_name = holder.getView(R.id.tv_book_name);
            ...
            if (null != getDynamicClickListener()) {
                dynamic_book_view.setOnClickListener(
                        v -> {
                            getDynamicClickListener().circleClick(holder.getAdapterPosition() - getHeaderLayoutCount());
                        }
                );
...
//                );
            ...
            /*开始数据加载*/
            /*通用部分*/
            RxImageTool.loadLogoImage(mContext, listBean.getPlatformCover(), iv_dynamic_book_cover);
            tv_dynamic_book_title.setText(listBean.getPlatformName());
            tv_dynamic_book_description.setText("已有" + listBean.getPlatformPeopleNumber() + "名用户加入了这个圈子");
            try {
                ArrayList<String> listTemp = new ArrayList<>();
                int size = 0;
                if (null != listBean.getHeadList() && !listBean.getHeadList().isEmpty()) {
                    size = listBean.getHeadList().size();
                    if (size > 3) {
                        listTemp.addAll(listBean.getHeadList().subList(0, 3));
                    } else {
                        listTemp.addAll(listBean.getHeadList());
                    }
                }
...
//                tv_dynamic_content_text.setText(listBean.getContentShow());
                URLUtils.replaceBook(listBean.getContentShow().replace("\n"," "),mContext,tv_dynamic_content_text,listBean.getUrlTitle());
//                tv_dynamic_content_text.post(
//                        new Runnable() {
//                            @Override
//                            public void run() {
//                                addEllipsisAndAllAtEnd(tv_dynamic_content_text);
//                            }
//                        }
//                );
            }else {
