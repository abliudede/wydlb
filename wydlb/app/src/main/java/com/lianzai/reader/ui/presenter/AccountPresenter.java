...
public class AccountPresenter extends RxPresenter<AccountContract.View> implements AccountContract.Presenter<AccountContract.View> {
    ...
    @Inject
    public AccountPresenter(ReaderApi readerApi) {
    ...
    public void getAccountDetail() {
        ...
        RxLogTool.e("getAccountDetail request...");
        Disposable disp =mReaderApi.getAccountDetail().compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getCode()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.getAccountDetailSuccess(data);
                                }else{
