...
public class MyDynamicFragment extends BaseFragment  implements  SwipeRefreshLayout.OnRefreshListener{
    ...
    public void circleDynamicRequest(boolean isRefresh){
        ...
        //是否上传用户id
        if(null != account){
            jsonObject.addProperty("dynamicUserId", account.getData().getUid());
        }
        ...
        public void moreOptionClick(int position,View view) {
            ...
            moreOptionPopup.setOnDismissListener(new PopupWindow.OnDismissListener() {
                ...
                public void onDismiss() {
                    //隐藏蒙层
                    if(getActivity() instanceof ActivityMyDynamic){
                        //圈子详情页
                        ((ActivityMyDynamic)getActivity()).getMask_view().setVisibility(View.GONE);
                    }
                }
            });
            //显示蒙层
            if(getActivity() instanceof ActivityMyDynamic){
                //圈子详情页
                ((ActivityMyDynamic)getActivity()).getMask_view().setVisibility(View.VISIBLE);
            }
