...
//        }
        ...
        if(null != im_iv_logo) {
            UserInfo userInfo = NimUIKit.getUserInfoProvider().getUserInfo(data.getFromAccount());
            if(null != userInfo) {
                RxImageTool.loadLogoImage(holder.getContext(), userInfo.getAvatar(), im_iv_logo);
            }
        }
...
//        }
        if(null != iv_retry) {
            ...
            iv_retry.setOnClickListener(
                    v -> {
                        teamChatAdapter.getChatItemClickListener().retrySendMsgClick(data);
                    }
            );
        }
        ...
        if(null != im_iv_logo) {
            ...
            im_iv_logo.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View view) {
                    teamChatAdapter.getChatItemClickListener().userLogoClick(data);
                    return true;
                }
            });
        }
        ...
        if(null != im_tv_timeline) {
            ...
            if (teamChatAdapter.needShowTime(data) || holder.getAdapterPosition() == 0) {//第一条消息要显示
