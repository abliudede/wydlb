...
import com.lianzai.reader.ui.contract.ChapterInfoContract;
...
public class ChapterInfoPresenter extends RxPresenter<ChapterInfoContract.View> implements ChapterInfoContract.Presenter<ChapterInfoContract.View> {
    ...
    @Override
    public void getChapterInfo(ArrayMap<String,Object> maps) {
        ...
        Disposable disp =mReaderApi.getChapterInfo(maps).compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getStatus_code()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.getChapterInfo(data);
                                }else{
...
//                            if (!e.toString().contains("NullPointerException")){
                                try{
                                    ...
                                    RxLogTool.e("ChapterInfoPresenter:"+ex.toString());
                                }
//                            }
                        }
                );
