...
public class RecordButtonYuD extends AppCompatTextView {
    ...
    public RecordButtonYuD(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public RecordButtonYuD(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void setDefaultFilePath() {
        ...
        File file = new File(dir, String.valueOf(System.currentTimeMillis())+ ".m4a");// + ".Ogg"
        if (!file.exists()) try {
            file.createNewFile();
        } catch (IOException e) {
            e.printStackTrace();
