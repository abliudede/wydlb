...
public class PublicNumberJoinTeamChatViewHolder extends RecyclerViewHolder<BaseQuickAdapter, BaseViewHolder, IMMessage> {

    public PublicNumberJoinTeamChatViewHolder(BaseQuickAdapter adapter) {
        super(adapter);
    }
    ...
    public void convert(BaseViewHolder holder, IMMessage data, int position, boolean isScrolling) {
        ...
        if (null!=data.getAttachment()&&data.getAttachment() instanceof PublicNumberAttachment){
            ...
            if(null != attachment.getPublicNumberBean()) {
                tv_chapter_title.setText(attachment.getPublicNumberBean().getTitle());
                tv_chapter_detail.setText(attachment.getPublicNumberBean().getContent());
                tv_open_current_chapter.setText("点击加入群聊");

                tv_open_current_chapter.setOnClickListener(
                        v -> {
                            ((PublicNumberAdapter) getAdapter()).getPublicNumberOptionCallback().joinTeamChat(attachment.getPublicNumberBean().getTeamId());
                        }
                );

                tv_chapter_title.setOnClickListener(
                        v -> {
                            ((PublicNumberAdapter) getAdapter()).getPublicNumberOptionCallback().joinTeamChat(attachment.getPublicNumberBean().getTeamId());
                        }
                );

                tv_chapter_detail.setOnClickListener(
                        v -> {
                            ((PublicNumberAdapter) getAdapter()).getPublicNumberOptionCallback().joinTeamChat(attachment.getPublicNumberBean().getTeamId());
                        }
                );
