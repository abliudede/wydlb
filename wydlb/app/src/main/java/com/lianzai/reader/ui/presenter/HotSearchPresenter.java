...
import com.lianzai.reader.ui.contract.HotSearchContract;
...
public class HotSearchPresenter extends RxPresenter<HotSearchContract.View> implements HotSearchContract.Presenter<HotSearchContract.View> {

    private ReaderApi mReaderApi;

    @Inject
    public HotSearchPresenter(ReaderApi readerApi) {
    ...
    public void hotSearch() {
        ...
        Disposable disp =mReaderApi.bookHotSearch().compose(RxUtils::toSimpleSingle)
                .subscribe(
                        data -> {
                            try{
                                if (data != null && mView != null && data.getStatus_code()== Constant.ResponseCodeStatus.SUCCESS_CODE) {
                                    mView.hotSearch(data);
                                }else{
